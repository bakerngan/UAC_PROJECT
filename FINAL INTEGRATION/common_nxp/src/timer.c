...
#ifdef MTHOMAS_MOD
volatile Timer_Callback_Func timer0CallbackFunction = NULL;
volatile Timer_Callback_Func timer1CallbackFunction = NULL;

DWORD registercallback_timer( BYTE timer_num, Timer_Callback_Func pFunction)
{
	BYTE ret;

	if ( pFunction != NULL ) {
		if ( timer_num == 0 ) {
			timer0CallbackFunction = pFunction;
			ret = TRUE;
		}
		else if ( timer_num == 1 ) {
			timer1CallbackFunction = pFunction;
			ret = TRUE;
		}
		else {
			ret = FALSE;
...
#ifdef MTHOMAS_MOD
	if ( timer1CallbackFunction != NULL ) {
